...
const Home = ({ navigation }) => {
    ...
    return (
        <Box py="$3" px="$2" marginTop="$10">
            ...
            <FlatList data={filteredNotes} renderItem={({ item }) => <ListNote key={item.noteId} judul={item.title} isi={item.content} tanggal="tanggal" status={item.status} category={item.category} noteId={item.noteId} />} keyExtractor={(item) => item.noteId} />
        </Box>
    );
};
export default Home;